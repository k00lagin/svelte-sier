<script>
		...
		switch (state) {
			...
			case "FIRST_NAME":
				if (searchComponents[1].length > 0) {
					...
					namePrompts = [
						...namePrompts,
						...getPopularFirstnames(searchComponents[1], searchGender),
					].filter(
						(name, index, names) =>
							names.findIndex((el) => el.value === name.value) === index
					);
				}
				break;
			case "MIDDLE_NAME":
	...
	function handleClientClick(e) {
		let client = [...recentClients, ...clients].filter(
			(client) => client.reestrId === e.currentTarget.getAttribute("data-id")
		)[0];
		...
		<div class="client-search-flyout">
			<ul>
				{#each filteredClients as client}
					<li
						on:click={handleClientClick}
						data-id={client._id || client.reestrId}
					>
						<span class="client-name"
							...
							{client.firstName}{client.middleName
								...
								: ""}</span
						>
						<span class="client-birthday"
							>{client.birthday ? client.birthday.formatted : ""}</span
						>
						<div class="client-document">
							{client.documentType
								? `${client.documentType[0].text} ${client.documentSeries} ${client.documentNumber}`
								: ""}
						</div>
					</li>
				{/each}
