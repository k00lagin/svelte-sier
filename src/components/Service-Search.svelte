	...
	function filterServices() {
		...
		allServices.forEach((source) => {
			...
			function match(priority, obj) {
				...
				if (obj.sid) {
					service.sidRich = service.sid
						.split(obj.sid)
						.join(`<b>${obj.sid}</b>`);
				}
				if (obj.name) {
					service.nameRich = service.name
						.split(obj.name)
						.join(`<b>${obj.name}</b>`);
				}
				if (obj.alias) {
					service.alias = $aliases[service.sid]
						.split(obj.alias)
						.join(`<b>${obj.alias}</b>`);
				}
			...
			let isEvery = words.every((word) => {
				...
				let caseIgnorantIdx = source.name
					.toLowerCase()
		...
		services = filteredServices.sort(
			(a, b) => b.matchPriority - a.matchPriority
		);
	}
			...
			case "Enter":
				if (
					document.querySelector(
						".service-search-dialog li.service-item:first-child > a"
					)
				) {
					document
						.querySelector(
			...
			{#each services as service, index (service.sid)}
				<li class="service-item">
					...
					<span class="service-code"
						>{@html service.sidRich || service.sid}</span
					>
					<a
						tabindex="0"
						class="service-link"
					...
					<span style="min-width: 52px;display: inline-block;">
						<Alias
							bind:value={$aliases[service.sid]}
							view={service.alias || $aliases[service.sid]}
							on:change={filterServices}
						/>
						<FavoriteButton
							on:change={handleFaveChange}
							value={service.sid}
							checked={$faves.indexOf(service.sid) !== -1}
						/>
					</span>
				</li>
			{:else}
